...
API_SERVER_OK = True  # REST API uses stdlib http.server — always available
...
SUSPICIOUS_PHRASES = {
    ...
    'conversation_hijack': (
        [r're:.*meeting', r'as we discussed', r'per our.*conversation',
         r'following up on.*our'], 10),  # Low weight — needs auth failure to matter
}
...
def run_scoring_engine(signals: List[ThreatSignal],
    ...
    # [v19] Escalation bonus now scales with cluster density (how many signals
    # fired within each required cluster). A rule with 1 weak signal per cluster
    # fires at ~75% of base bonus; 4+ signals per cluster fires at 100%.
    # This prevents a single SPF fail + single BEC signal from triggering
    # the same CRITICAL escalation as a full 4-signal attack pattern.
...
def build_risk_narrative(signals: List[ThreatSignal],
    ...
    if 'steganography' in sig_names:
        ...
        _add_ev(f"contains an image with hidden steganographic data"
                f"{' (' + steg_sig.evidence + ')' if steg_sig and steg_sig.evidence else ''}")
...
def extract_inner_url(url: str) -> Optional[str]:
    ...
    try:
        ...
        params = parse_qs(qs)
        # Common parameter names used by gateways for the real destination
        _INNER_PARAM_NAMES = (
...
def check_typosquat(domain):
    ...
    allowed_variants = {
        ...
        'microsoft': {
            ...
            # Microsoft 365 tenant domains — every M365 customer gets a *.onmicrosoft.com
            # subdomain. These are Microsoft-controlled and legitimate.
            'onmicrosoft.com',
            # Other Microsoft infrastructure commonly seen in email headers/links
...
class Config:
    ...
    try:
        ...
        _key_seed = hashlib.md5(os.environ.get("COMPUTERNAME", socket.gethostname()).encode()).hexdigest()
...
def generate_signals(auth, anomalies, reply_hijack, bec, spoof_result, spoof_dn, spoof_sd,
    ...
    for o in observables:
        ...
        if o.vt and o.vt.domain_intel and not o.vt.domain_intel.trusted:
            ...
            if not is_tracking_domain(dom_to_check) and not is_tracking_domain(_root_domain(dom_to_check)):
                sub_ent = subdomain_entropy(dom_to_check)
                if sub_ent > 4.0:   # Raised from 3.5 to eliminate structured hostname FPs
                    sigs.append(ThreatSignal('dga_domain_entropy', 'content', 3, 0.35, 0.70,
                        ...
                        o.defanged))
                if is_randomized_domain(dom_to_check):
...
def build_reasoning_cards(R):
    ...
    cards.append(('\U0001f4ca', 'Scan Coverage',
        ...
        f"**{len([m for m in R['macros'] if m.filename != '[Body]'])}** file(s) + "
        f"**{len([m for m in R['macros'] if m.filename != '[Body]'])}** file(s) via YARA + MalwareBazaar | "
...
[data-testid="stMetricValue"]{{font-size:2em;font-weight:700;background:linear-gradient(135deg,{DC['accent']},{DC['purple']});-webkit-background-clip:text;-webkit-text-fill-color:transparent}}
    ...
    with st.sidebar:
        ...
        st.metric("REST API",
                  f"\u2705 :{Config.API_PORT}" if API_SERVER_OK else "\u26a0\ufe0f Offline")
        ...
        if API_SERVER_OK:
            ...
            st.caption(f"API: `http://localhost:{Config.API_PORT}/`")
 ...
 # ══════════════════════════════════════════════════════════════════
        ...
        with tabs[8]:
            ...
            if API_SERVER_OK:
                ...
                st.warning(
                    "\u26a0\ufe0f REST API offline. Check server logs for errors.")
